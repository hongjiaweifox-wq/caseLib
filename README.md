...
主页 Tab：**知识图谱** · **家庭参数** · **实时运行**（接线拓扑编辑已并入实时运行画布）。
...
- PV/负载/离网曲线 + 防逆流，写入同一 `lab_*.json` 的 `scene`
  ...
  - **参数模板库**：左栏顶「参数模板」卡片，把整套家庭参数（PV含路数/家庭/离网/防逆流 + 曲线控制点）**另存为命名模板**、下拉切换即载入、保存到所选更新、删除。存 `data/scene_templates.json`，与接线家庭解耦。接口 `GET /api/scene/templates`、`POST /api/scene/template/save|delete`，模块 `caseLib/scene_templates.py`
